...
function Resolve-ProcNameFromPid([int]$ProcPid){
  $dbgDir = "C:\ProgramData\GuardianFW\events"
  $dbg    = Join-Path $dbgDir "pidresolve.debug.txt"
  try { New-Item -ItemType Directory -Path $dbgDir -Force | Out-Null } catch {}

  if($ProcPid -le 0){
    try { Add-Content -LiteralPath $dbg -Value ("[{0}] pid={1} -> unknown (pid<=0)" -f (Get-Date).ToString("o"), $ProcPid) -Encoding UTF8 } catch {}
    return "unknown"
  }

  try { if($null -eq $global:PidMap){ $global:PidMap = [hashtable]::Synchronized(@{}) } } catch {}

  # If we already learned it, return immediately (no waiting)
  try {
    if($global:PidMap.ContainsKey($ProcPid)){
      $v = [string]$global:PidMap[$ProcPid]
      if(-not [string]::IsNullOrWhiteSpace($v)){
        $v = $v.ToLowerInvariant()
        try { Add-Content -LiteralPath $dbg -Value ("[{0}] pid={1} -> {2} (PidMap immediate)" -f (Get-Date).ToString("o"), $ProcPid, $v) -Encoding UTF8 } catch {}
        return $v
      }
    }
  } catch {}
  ...
  # FAST path: Get-Process immediately (do not wait)
  try {
    $p = Get-Process -Id $ProcPid -ErrorAction Stop
    $n = & $Norm ([string]$p.ProcessName)
    if($n){
      try { $global:PidMap[$ProcPid] = $n } catch {}
      try { Add-Content -LiteralPath $dbg -Value ("[{0}] pid={1} -> {2} (Get-Process immediate)" -f (Get-Date).ToString("o"), $ProcPid, $n) -Encoding UTF8 } catch {}
      return $n
    }
  } catch {
    try { Add-Content -LiteralPath $dbg -Value ("[{0}] pid={1} Get-Process failed: {2}" -f (Get-Date).ToString("o"), $ProcPid, $_.Exception.Message) -Encoding UTF8 } catch {}
  }

  # CIM fallback (robust)
  try {
    $c  = Get-CimInstance Win32_Process -Filter ("ProcessId=" + $ProcPid) -ErrorAction Stop | Select-Object -First 1
    $cn = $c | Select-Object -ExpandProperty Name -ErrorAction Stop
    $n  = & $Norm ([string]$cn)
    if($n){
      try { $global:PidMap[$ProcPid] = $n } catch {}
      try { Add-Content -LiteralPath $dbg -Value ("[{0}] pid={1} -> {2} (CIM)" -f (Get-Date).ToString("o"), $ProcPid, $n) -Encoding UTF8 } catch {}
      return $n
    }
  } catch {
    try { Add-Content -LiteralPath $dbg -Value ("[{0}] pid={1} CIM failed: {2}" -f (Get-Date).ToString("o"), $ProcPid, $_.Exception.Message) -Encoding UTF8 } catch {}
  }

  try { Add-Content -LiteralPath $dbg -Value ("[{0}] pid={1} -> unknown (all failed)" -f (Get-Date).ToString("o"), $ProcPid) -Encoding UTF8 } catch {}
  return "unknown"
}
function Resolve-PidFromRemoteTcp([string]$RemoteIp, [int]$RemotePort, [int]$Tries=15, [int]$SleepMs=40) {
  for($i=0; $i -lt $Tries; $i++){
    try {
      $c = Get-NetTCPConnection -RemoteAddress $RemoteIp -RemotePort $RemotePort -State Established,SynSent,SynReceived -EA SilentlyContinue |
...
$global:PidMap = [hashtable]::Synchronized(@{})
    ...
    foreach($p in Get-Process -ErrorAction SilentlyContinue){
      try {
        ...
        if($id -gt 0){
          ...
          if($n){
            if(-not $n.EndsWith(".exe",[System.StringComparison]::OrdinalIgnoreCase)){ $n = $n + ".exe" }
            $global:PidMap[$id] = $n.ToLowerInvariant()
          }
        }
      } catch {}
    }
...
function Start-PidPoller([int]$Seconds, [int]$IntervalMs = 200){
  try {
    if($null -eq $global:PidMap){ $global:PidMap = [hashtable]::Synchronized(@{}) }
...
function Consume-PidSpool(){
  try {
    $spoolFile = "C:\ProgramData\GuardianFW\events\pidwatch.jsonl"
    if(!(Test-Path -LiteralPath $spoolFile)){ return }

    $lines = @(Get-Content -LiteralPath $spoolFile -Encoding UTF8 -ErrorAction SilentlyContinue)
    if($lines.Count -gt 0){
      try { Clear-Content -LiteralPath $spoolFile -ErrorAction SilentlyContinue } catch {}
...
function Start-PidWatcher(){
  try {
    # WMI events are unreliable on some systems; use polling instead.
    $dir = "C:\ProgramData\GuardianFW\events"
    $sp  = Join-Path $dir "pidwatch.jsonl"
    try { New-Item -ItemType Directory -Path $dir -Force | Out-Null } catch {}
    try { New-Item -ItemType File -Path $sp -Force | Out-Null } catch {}
  } catch {}
}
...
function Stop-PidWatcher(){
  # polling has nothing to stop; kept for compatibility
}
if($NoRun){ return }
...
try {
  ...
  if($SelfTest){
    ...
    # keep PID->process mapping fresh during capture window (no WMI)
    try { Start-PidPoller -Seconds $WindowSeconds -IntervalMs 200 } catch {}
    # --- PID poller runs during capture window (no WMI) ---
    $pidPollJob = $null
    try {
      $pidPollJob = Start-Job -ScriptBlock {
        param($seconds)
        try {
          if($null -eq $global:PidMap){ $global:PidMap = [hashtable]::Synchronized(@{}) }
          Start-PidPoller -Seconds $seconds -IntervalMs 200
        } catch {}
      } -ArgumentList $WindowSeconds
    } catch {}
  Start-Sleep -Seconds $WindowSeconds
    try {
      if($pidPollJob){ Stop-Job $pidPollJob -Force -EA SilentlyContinue | Out-Null; Remove-Job $pidPollJob -Force -EA SilentlyContinue | Out-Null }
    } catch {}
    # --- end PID poller ---
}
finally {
...
foreach($ip in $dohIps){
  ...
  foreach($m in $reBlock.Matches($blob)){
    ...
    # If source is preconnect, PID may be unreliable. Correlate via TCP table.
    if($source -eq "etw_tcpip_preconnect"){
      # Use $ip from foreach($ip in $dohIps) and fixed TLS port 443
      $port = 443
      $realPid = Resolve-PidFromRemoteTcp -RemoteIp $ip -RemotePort $port -Tries 30 -SleepMs 25
      if($realPid -gt 0){
        $procId = $realPid
        $proc   = Resolve-ProcNameFromPid $procId
      } else {
        $proc = "preconnect"
      }
    } else {
      if($procId -gt 0){ $proc = Resolve-ProcNameFromPid $procId } else { $proc = "unknown" }
    }

    # Best practice: resolve proc name from PID safely + "learn" it into the PID map.
    try { Consume-PidSpool } catch {}
    if($procId -gt 0){
      try {
        $pn = "unknown"
        ...
        try {
          if($global:PidMap.ContainsKey($procId)){
            $pn = [string]$global:PidMap[$procId]
          }
        } catch {}
        ...
        if(-not [string]::IsNullOrWhiteSpace($pn) -and $pn -ne "unknown"){
          if(([string]$proc).ToLowerInvariant() -eq "unknown" -and -not [string]::IsNullOrWhiteSpace($pn) -and $pn.ToLowerInvariant() -ne "unknown"){
            $proc = $pn.ToLowerInvariant()
          }
          try { $global:PidMap[$procId] = $pn } catch {}
        }
      } catch {}
    }
