...
function Merge-ExitCode([int]$current, [int]$incoming){
  # 0=OK, 20=DRIFT, 50+=ERROR
  if($incoming -ge 50){ return 50 }
  if($incoming -eq 20 -and $current -lt 20){ return 20 }
  return $current
}
...
function Run-Probe_Legacy([string]$ProbePath){
  ...
  try {
    ...
    $code = $LASTEXITCODE
$overallExit = Merge-ExitCode $overallExit $code
  } catch {
    $code = 70
...
# --- GFW_PROBE_TIMEOUT_V2 ---
function Run-Probe([string]$ProbePath, [int]$TimeoutSec = 25){

  $outFile  = Join-Path $env:TEMP ("gfw_probe_out_{0}.txt" -f ([guid]::NewGuid().ToString("N")))
  $errFile  = Join-Path $env:TEMP ("gfw_probe_err_{0}.txt" -f ([guid]::NewGuid().ToString("N")))
  $exitFile = Join-Path $env:TEMP ("gfw_probe_exit_{0}.txt" -f ([guid]::NewGuid().ToString("N")))

  $code = 0

  $job = Start-Job -ScriptBlock {
    param($ProbePath,$outFile,$errFile,$exitFile)
    $code = 0
    try {
      & powershell.exe -NoProfile -ExecutionPolicy Bypass -File $ProbePath 1> $outFile 2> $errFile
      $code = $LASTEXITCODE
    ...
    try { Set-Content -LiteralPath $exitFile -Value ([string]$code) -Encoding ASCII -Force } catch {}
  } -ArgumentList $ProbePath,$outFile,$errFile,$exitFile
  ...
  if(-not $finished){
    ...
    $code = 80
    try { Set-Content -LiteralPath $errFile -Value ("TIMEOUT after {0}s: {1}" -f $TimeoutSec,$ProbePath) -Encoding UTF8 -Force } catch {}
  } else {
    try { Remove-Job $job -Force | Out-Null } catch {}
    try {
      if(Test-Path -LiteralPath $exitFile){
        $code = [int](Get-Content -LiteralPath $exitFile -Raw -Encoding ASCII)
      } else {
        $code = 70
      }
    } catch { $code = 70 }
  ...
  try { Remove-Item -LiteralPath $outFile,$errFile,$exitFile -Force -ErrorAction SilentlyContinue | Out-Null } catch {}

  return @{ exit = [int]$code; out = $out; err = $err }
}
# --- END GFW_PROBE_TIMEOUT_V2 ---
