...
if($bad){
  ...
  try {
    ...
    foreach($c in $tcp443){
      ...
      if(-not $rip){ continue }
      __DBG "DBG_BEFORE_TCP443_BLOCK"
      __DBG "DBG_AFTER_TCP443_MARK"

      try {
        $c = @(Get-NetTCPConnection -State Established -EA SilentlyContinue | Where-Object { $_.RemotePort -eq 443 }).Count
        __DBG ("DBG_TCP443_CONN_COUNT " + $c)
      } catch { __DBG ("DBG_TCP443_CONN_COUNT_EX " + $_.Exception.Message) }
...
try {
  ...
  __DBG "DBG_ENTRY_WARMUP"
  try {
    $k = ("taskwarmup|{0}|{1}" -f $env:COMPUTERNAME, $PID)
    $r = Should-LogEvent $k
    __DBG ("DBG_ENTRY_WARMUP_OK allow={0} key={1}" -f $r, $k)
    try { Write-Audit "shouldlog_taskwarmup" @{ key=$k; allow=$r; pid=$PID; user=$env:USERNAME } } catch {}
  } catch {
    __DBG ("DBG_ENTRY_WARMUP_EX {0}" -f $_.Exception.Message)
    try { Write-Audit "shouldlog_taskwarmup_failed" @{ error=$_.Exception.Message } } catch {}
  }
