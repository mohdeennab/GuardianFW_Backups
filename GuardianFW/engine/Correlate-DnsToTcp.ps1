...
# PS 5.1-safe: precompute source label (avoid inline if/else inside hashtables)
$src = "dns_cache_tail"
try {
  if ($script:DnsCachePath -and (Test-Path -LiteralPath $script:DnsCachePath)) { $src = "dns_ip_index" }
} catch { $src = "dns_cache_tail" }
  ...
  if ($null -ne $strong) {
    ...
    return [pscustomobject]@{
      ...
      dns=[pscustomobject]@{ domain=$strong.domain; resolver=$strong.resolver; qtype=$strong.qtype; resolution_age_ms=$age; source=$src }
      window_ms=$script:LookbackMs; dst_ip=$DstIP; dst_port=$DstPort; pid=$ProcPid; proc=$ProcName; protocol=$Protocol; ts=$EventTsUtcIso
    }
  }
  ...
  [pscustomobject]@{
    ...
    dns=[pscustomobject]@{ domain=$weak.domain; resolver=$weak.resolver; qtype=$weak.qtype; resolution_age_ms=$ageW; source=$src }
    window_ms=$script:LookbackMs; dst_ip=$DstIP; dst_port=$DstPort; pid=$ProcPid; proc=$ProcName; protocol=$Protocol; ts=$EventTsUtcIso
  }
}
