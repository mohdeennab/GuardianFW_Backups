...
function Get-DedupeKey([string]$ev,$data){
  ...
  if($data){
    foreach($k in ($data.Keys | Sort-Object)){
      # SPECIAL CASE: tcp443_seen  ignore remote IP (reduce noise)
      if($ev -eq "tcp443_seen" -and $k -eq "remote"){ continue }

      # OPTIONAL: ignore pid too for per-app-only throttling
      # if($ev -eq "tcp443_seen" -and $k -eq "pid"){ continue }

      $v = ""
      try { $v = ([string]$data[$k]).Trim().ToLowerInvariant() } catch { $v = "" }
      $pairs += ("{0}={1}" -f $k.ToLowerInvariant(), $v)
    }
  }
  ...
  try{
    return (-join ($sha.ComputeHash([Text.Encoding]::UTF8.GetBytes($raw)) | ForEach-Object { Set-StrictMode -Version Latest
$ErrorActionPreference = "Stop"
