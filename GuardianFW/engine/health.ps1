...
$health = "OK"
if($exit -eq 20){ $health = "DRIFT" }
elseif($exit -ge 50){ $health = "ERROR" }

Write-Output ("HEALTH: {0}  (firewall={1}, registry={2}, integrity={3})" -f $health, $fw.status, $reg.status, $int.status)
exit $exit
